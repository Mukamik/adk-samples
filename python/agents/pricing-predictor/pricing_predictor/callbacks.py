...
def add_step_pricing_summary(
    ...
    extrapolated_cost = step_cost_float * 1000
    ...
    step_summary_str = (
        ...
        f"After 1,000 runs: ${extrapolated_cost:.6f}\n"
    )
...
def add_final_summary(callback_context: CallbackContext) -> Optional[types.Content]:
    ...
    if not total_tokens_list:
        return None # No tokens were used, so do nothing.
    ...
    extrapolated_cost = total_cost_float * 1000
    ...
    # Get the original final output from the state.
    original_output = current_state.get("output", "")
    
    # Combine the original output with our new summary.
    new_output_text = original_output + final_summary_str
    
    # Return a new Content object to *replace* the agent's original output.
